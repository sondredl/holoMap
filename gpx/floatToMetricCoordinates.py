...
def parse_and_convert(json_file, output_file):
    ...
    first_lat = data[0]['lat']
    first_lon = data[0]['lon']
    ...
    for i in range(1, len(data)):
        prev_lat = data[i - 1]['lat'] if i > 1 else first_lat
        prev_lon = data[i - 1]['lon'] if i > 1 else first_lon
        current_lat = data[i]['lat']
        current_lon = data[i]['lon']
